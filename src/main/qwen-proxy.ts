...
export class QwenProxy {
  private app: http.Server | null = null;
  private mainWindow: BrowserWindow | null = null;
  private session: QwenSession | null = null;
  private port: number;
  ...
  setWindow(win: BrowserWindow) {
    this.mainWindow = win;
    logger.info('🔐 QwenProxy: Window attached, session stealer active');
  }
   ...
   */
  private async stealSession(): Promise<QwenSession | null> {
    ...
    try {
      ...
      logger.error('❌ Failed to steal session', err);
      return null;
    }
  }
   ...
   */
  private async createChatId(sess: QwenSession): Promise<string> {
   ...
   */
  private async forwardToQwen(
    sess: QwenSession,
    chatId: string,
    payload: any
  ): Promise<any> {
    return new Promise((resolve, reject) => {
      const url = new URL(QWEN_ENDPOINT, QWEN_BASE);
        ...
        if (isStream) {
            ...
            for (const line of lines) {
              if (line.startsWith('data: ')) {
                ...
                try {
                  ...
                  logger.debug('SSE parse skip:', jsonStr.slice(0, 100));
                }
              }
            }
            
            // Convert Qwen stream chunks to OpenAI format
            const openaiChunks = results.map((chunk: any, idx: number) => {
          ...
          res.on('end', () => {
            try {
              ...
              logger.info('🔍 RAW QWEN RESPONSE:', rawStr.slice(0, 3000));
              ...
              if (Array.isArray(content)) {
                content = content.map((c: any) => c.text || c.content || c).join('\n');
              }
              ...
              logger.info(`📝 Extracted content length: ${content?.length || 0}`);
              if (!content && rawStr.length > 0) {
                logger.warn('⚠️ Content extraction failed! Available keys:', Object.keys(qwenResp).join(', '));
              }
              // === END DEBUG ===
              ...
              logger.error('Raw response:', data.slice(0, 500));
      ...
      req.on('error', (err) => {
        logger.error('❌ Qwen API request failed:', err);
        reject(err);
      });
  ...
  start() {
      ...
      if (req.url?.startsWith('/v1/chat/completions') && req.method === 'POST') {
        ...
        req.on('end', async () => {
          try {
            ...
            logger.info(`📨 OpenAI request: model=${payload.model}, stream=${payload.stream}`);
    ...
    this.app.listen(this.port, 'localhost', () => {
      logger.info(`🚀 QwenProxy listening on http://localhost:${this.port}`);
      logger.info(`📡 Endpoint: POST /v1/chat/completions (OpenAI-compatible)`);
    });
  ...
  stop() {
    if (this.app) {
      this.app.close();
      this.app = null;
      logger.info('🛑 QwenProxy stopped');
    }
  }
}

export const qwenProxy = new QwenProxy();
